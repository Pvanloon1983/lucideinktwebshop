...
class PickupLocationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($limit <= 0) $limit = 60; elseif ($limit > 200) $limit = 200;

        // Map numeric carrier ids to names
        $rawCarrier = trim((string) $request->input('carrier', ''));
        $carrierMap = ['1' => 'postnl', '2' => 'bpost', '4' => 'dpd'];
        if (isset($carrierMap[$rawCarrier])) $rawCarrier = $carrierMap[$rawCarrier];
        $requestedCarrier = strtolower($rawCarrier);
        if (!in_array($requestedCarrier, ['postnl','bpost','dpd'])) $requestedCarrier = '';
        ...
        foreach ($carriers as $carrier) {
            ...
            try {
                ...
                foreach ((array) $raw as $p) {
                    ...
                    if (count($all) >= $limit) break 2; // stop if reached limit
                }
            } catch (\Throwable $e) {
