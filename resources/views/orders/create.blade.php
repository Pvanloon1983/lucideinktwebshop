		...
		<div class="alert alert-success" style="position: relative;">
			...
			{{-- @if(session('payment_link'))
				<div style="margin-top:8px;">
					<a href="{{ session('payment_link') }}" target="_blank" rel="noopener" class="btn">Betaal deze bestelling</a>
				</div>
			@endif --}}
		...
		<form class="form" action="{{ route('orderStore') }}" method="POST">
			...
			<div class="order-grid">
					...
					<ul>
						...
						<li>
							<div class="input-box">
								...
								<div class="quantity">
									<span>Aantal: </span>
									<input type="number" name="items[{{ $product->id }}][qty]" id="product_{{ $product->id }}"
										value="{{ old($inputName, 0) }}" min="0" class="qty-input" data-price="{{ $product->price }}"
										data-id="{{ $product->id }}">
				...
				<div class="item discount_value">
					...
					@php
						// If there are no errors and no old input, reset discount fields to 0 and 'amount'
					...
					<div class="discounted-total" id="discounted-total">
					</div>
				</div>
